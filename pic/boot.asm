...
boot_init
    ...
    movlw       high clock_jump_table
    movwf       high_jump

    movf        eeprom_data, w
    andlw       0x03            ; make sure it's a valid clock setting

    rlncf       WREG, w
    rlncf       WREG, w

    addlw       low clock_jump_table
    btfsc       STATUS, C
    incf        high_jump, f
...
clock_1                         ; 2.5MHz
; set up the PWM peripheral that's going to generate the clock
    movlw       d'3'            ; Frequency setting  1 = 5MHz, 2 = 3.33MHz, 
                                 ;                    3 = 2.5MHz, 39 = 250kHz
    movwf       PR2
; Duty cycle 1/2 
; 10 bit value bits <9:2> in CCPR1L
...
clock_2                         ; 3.33MHz
; set up the PWM peripheral that's going to generate the clock
    movlw       d'2'            ; Frequency setting  1 = 5MHz, 2 = 3.33MHz, 
                                 ;                    3 = 2.5MHz, 39 = 250kHz
    movwf       PR2
; Duty cycle 1/2 
; 10 bit value bits <9:2> in CCPR1L
...
clock_3                         ; 5MHz
; set up the PWM peripheral that's going to generate the clock
    movlw       d'1'            ; Frequency setting  1 = 5MHz, 2 = 3.33MHz, 
                                 ;                    3 = 2.5MHz, 39 = 250kHz
    movwf       PR2
; Duty cycle 1/2 
; 10 bit value bits <9:2> in CCPR1L
